...
<body>
  ...
  <br><br>
        ...
        <div class="card-deck mb-3 text-center">
          <div class="card mb-4 box-shadow">
            <div class="card-header">
              <h4 class="my-0 font-weight-normal">{{$bici->nombre}}</h4>
            </div>
            <div class="card-body">
              <ul class="list-unstyled mt-3 mb-4">
                <li><strong>Tipo: </strong>{{$bici->tipo}}</li>
                <li><strong>Medidas: </strong>{{$bici->medidas}}</li>
                <li><strong>Color: </strong>{{$bici->color}}</li>
                <li><strong>Peso: </strong>{{$bici->peso}}</li>
                <li><strong>Precio: </strong> {{$bici->precio}}€</li>
                <img src="{{$bici->imagen}}" width=300 height=200>
              </ul>
              <a href="{{ route('almacenarproducto', ['id' => $bici->id, 'tipo' => 'bici'])}}"><button id="boton" type="button" class="btn btn-lg btn-block btn btn-outline-secondary">Añadir al carrito</button></a>
            </a>
            </div>
          </div>
          </div>
    ...
    @if (Route::has('login'))
    @auth
      <form action="{{ route('almacenaresenya', ['id' => $bici->id, 'tipo' => 'bici']) }}" method="POST" class="container mt-4">
        @csrf
        ...
        <button type="submit" class=" btn-block btn btn-outline-secondary">Publicar reseña</button>
        <br><br>
    </form>
  @else
  <div class="center">
